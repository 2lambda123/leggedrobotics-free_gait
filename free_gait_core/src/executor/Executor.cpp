...
Executor::Executor(StepCompleter &completer, StepComputer &computer,
                   AdapterBase &adapter, State &state)
    : completer_(completer), computer_(computer), adapter_(adapter),
      state_(state), isInitialized_(false), isReset_(false), isPausing_(false),
      preemptionType_(PreemptionType::PREEMPT_STEP), queue_(),
...
bool Executor::isInitialized() const { return isInitialized_; }

Executor::Mutex &Executor::getMutex() { return mutex_; }
...
bool Executor::advance(double dt, bool skipStateMeasurmentUpdate) {
  ...
  if (executionStatus) {
    if (!state_.getRobotExecutionStatus())
      addToFeedback("Continuing with execution.");
    state_.setRobotExecutionStatus(true);
  } else {
    if (state_.getRobotExecutionStatus()) {
      if (!adapter_.isExecutionOk())
        addToFeedback("Robot status is not OK, paused execution.");
      if (isPausing_)
        addToFeedback("Paused execution.");
    }
  ...
  if (!queue_.empty() && queue_.getCurrentStep().needsComputation() &&
      computer_.isDone()) {
...
void Executor::pause(bool shouldPause) { isPausing_ = shouldPause; }
...
const StepQueue &Executor::getQueue() const { return queue_; }

StepQueue &Executor::getQueue() { return queue_; }

const State &Executor::getState() const { return state_; }

const AdapterBase &Executor::getAdapter() const { return adapter_; }
...
bool Executor::resetStateWithRobot() {
  ...
  if (state_.getNumberOfSupportLegs() > 0) {
    state_.setControlSetup(BranchEnum::BASE,
                           adapter_.getControlSetup(BranchEnum::BASE));
  } else {
  ...
  //  state_.setAllJointAccelerations(adapter_.getAllJointAccelerations()); //
  //  TODO
  state_.setAllJointEfforts(adapter_.getAllJointEfforts());
  state_.setPositionWorldToBaseInWorldFrame(
      adapter_.getPositionWorldToBaseInWorldFrame());
  state_.setOrientationBaseToWorld(adapter_.getOrientationBaseToWorld());
  state_.setLinearVelocityBaseInWorldFrame(
      adapter_.getLinearVelocityBaseInWorldFrame());
  state_.setAngularVelocityBaseInBaseFrame(
      adapter_.getAngularVelocityBaseInBaseFrame());
  state_.setRobotExecutionStatus(true);
...
bool Executor::updateStateWithMeasurements() {
  for (const auto &limb : adapter_.getLimbs()) {
    ...
    if (!controlSetup.at(ControlLevel::Position)) {
      state_.setJointPositionsForLimb(limb,
                                      adapter_.getJointPositionsForLimb(limb));
    }
    if (!controlSetup.at(ControlLevel::Velocity)) {
      state_.setJointVelocitiesForLimb(
          limb, adapter_.getJointVelocitiesForLimb(limb));
    }
    if (!controlSetup.at(ControlLevel::Acceleration)) {
      //      state_.setJointAccelerations(limb,
      //      adapter_.getJointAccelerations(limb));
    }
    if (!controlSetup.at(ControlLevel::Effort)) {
      state_.setJointEffortsForLimb(limb,
                                    adapter_.getJointEffortsForLimb(limb));
    }
  }
...
bool Executor::writeIgnoreForPoseAdaptation() {
  ...
  for (const auto &limb : adapter_.getLimbs()) {
    if (step.hasLegMotion(limb)) {
      bool ignoreForPoseAdaptation =
          step.getLegMotion(limb).isIgnoreForPoseAdaptation();
  ...
  for (const auto &limb : adapter_.getLimbs()) {
    if (step.hasLegMotion(limb)) {
      if (step.getLegMotion(limb).hasSurfaceNormal()) {
        state_.setSurfaceNormal(limb,
                                step.getLegMotion(limb).getSurfaceNormal());
      } else {
        state_.removeSurfaceNormal(limb);
      }
    }
  ...
  for (const auto &limb : adapter_.getLimbs()) {
    if (step.hasLegMotion(limb)) {
      if (step.getLegMotion(limb).hasFrictionCoefficient()) {
        state_.setFrictionCoefficient(
            limb, step.getLegMotion(limb).getFrictionCoefficient());
      } else {
        state_.removeFrictionCoefficient(limb);
      }
    }
...
bool Executor::writeLegMotion() {
  ...
  for (const auto &limb : adapter_.getLimbs()) {
    ...
    case LegMotionBase::TrajectoryType::EndEffector: {
      const auto &endEffectorMotion =
          dynamic_cast<const EndEffectorMotionBase &>(legMotion);
      if (controlSetup[ControlLevel::Position]) {
        const std::string &frameId =
            endEffectorMotion.getFrameId(ControlLevel::Position);
        if (!adapter_.frameIdExists(frameId)) {
          std::cerr << "Could not find frame '" << frameId
                    << "' for free gait leg motion!" << std::endl;
          return false;
        }
        Position positionInBaseFrame = adapter_.transformPosition(
            frameId, adapter_.getBaseFrameId(),
            endEffectorMotion.evaluatePosition(time));
        JointPositionsLeg jointPositions;
        if (!adapter_.getLimbJointPositionsFromPositionBaseToFootInBaseFrame(
                positionInBaseFrame, limb, jointPositions)) {
          std::cerr << "Failed to compute joint positions from end effector "
                       "position for "
                    << limb << "." << std::endl;
          return false;
        }
      ...
      if (controlSetup[ControlLevel::Velocity]) {
        const std::string &frameId =
            endEffectorMotion.getFrameId(ControlLevel::Velocity);
        if (!adapter_.frameIdExists(frameId)) {
          std::cerr << "Could not find frame '" << frameId
                    << "' for free gait leg motion!" << std::endl;
          return false;
        }
        // TODO This is dangerous due to difference between relative velocity
        // vs. expression in frames.
        LinearVelocity velocityInWorldFrame = adapter_.transformLinearVelocity(
            frameId, adapter_.getWorldFrameId(),
            endEffectorMotion.evaluateVelocity(time));
        const JointVelocitiesLeg jointVelocities =
            adapter_
                .getJointVelocitiesFromEndEffectorLinearVelocityInWorldFrame(
                    limb, velocityInWorldFrame);
        state_.setJointVelocitiesForLimb(limb, jointVelocities);
      }
      if (controlSetup[ControlLevel::Acceleration]) {
        const std::string &frameId =
            endEffectorMotion.getFrameId(ControlLevel::Acceleration);
        if (!adapter_.frameIdExists(frameId)) {
          std::cerr << "Could not find frame '" << frameId
                    << "' for free gait leg motion!" << std::endl;
          return false;
        }
        LinearAcceleration accelerationInWorldFrame =
            adapter_.transformLinearAcceleration(
                frameId, adapter_.getWorldFrameId(),
                endEffectorMotion.evaluateAcceleration(time));
        const JointAccelerationsLeg jointAccelerations =
            adapter_
                .getJointAccelerationsFromEndEffectorLinearAccelerationInWorldFrame(
                    limb, accelerationInWorldFrame);
    ...
    case LegMotionBase::TrajectoryType::Joints: {
      const auto &jointMotion =
          dynamic_cast<const JointMotionBase &>(legMotion);
      if (controlSetup[ControlLevel::Position])
        state_.setJointPositionsForLimb(limb,
                                        jointMotion.evaluatePosition(time));
      if (controlSetup[ControlLevel::Velocity])
        state_.setJointVelocitiesForLimb(limb,
                                         jointMotion.evaluateVelocity(time));
      if (controlSetup[ControlLevel::Acceleration])
        state_.setJointAccelerationsForLimb(
            limb, jointMotion.evaluateAcceleration(time));
      if (controlSetup[ControlLevel::Effort])
        state_.setJointEffortsForLimb(limb, jointMotion.evaluateEffort(time));
      break;
    }

    default:
      throw std::runtime_error("Executor::writeLegMotion() could not write leg "
                               "motion of this type.");
      break;
    }
  }
...
bool Executor::writeTorsoMotion() {
  ...
  if (!queue_.getCurrentStep().hasBaseMotion())
    return true;
  double time = queue_.getCurrentStep().getTime();
  ...
  if (controlSetup[ControlLevel::Position]) {
    ...
    if (!adapter_.frameIdExists(frameId)) {
      std::cerr << "Could not find frame '" << frameId
                << "' for free gait base motion!" << std::endl;
      return false;
    }
    Pose poseInWorldFrame = adapter_.transformPose(
        frameId, adapter_.getWorldFrameId(), baseMotion.evaluatePose(time));
  ...
  if (controlSetup[ControlLevel::Velocity]) {
    ...
    LinearVelocity linearVelocityInWorldFrame =
        adapter_.transformLinearVelocity(frameId, adapter_.getWorldFrameId(),
                                         twist.getTranslationalVelocity());
    LocalAngularVelocity angularVelocityInBaseFrame =
        adapter_.transformAngularVelocity(frameId, adapter_.getBaseFrameId(),
