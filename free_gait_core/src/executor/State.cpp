...
State::State() : QuadrupedState(), robotExecutionStatus_(false) {}
...
bool State::getRobotExecutionStatus() const { return robotExecutionStatus_; }

void State::setRobotExecutionStatus(bool robotExecutionStatus) {
  robotExecutionStatus_ = robotExecutionStatus;
}

const std::string &State::getStepId() const { return stepId_; }

void State::setStepId(const std::string &stepId) { stepId_ = stepId; }
...
void State::setSurfaceNormal(const LimbEnum &limb,
...
void State::setFrictionCoefficient(const LimbEnum &limb,
...
void State::setIgnoreForPoseAdaptation(const LimbEnum &limb,
                                       bool ignorePoseAdaptation) {
  ignoreForPoseAdaptation_[limb] = ignorePoseAdaptation;
}

const JointPositionsLeg
State::getJointPositionsForLimb(const LimbEnum &limb) const {
  return JointPositionsLeg(
      quadruped_model::QuadrupedState::getJointPositions()
          .vector()
          .segment<QD::getNumDofLimb()>(QD::getLimbStartIndexInJ(limb)));
}

void State::setJointPositionsForLimb(const LimbEnum &limb,
                                     const JointPositionsLeg &jointPositions) {
  quadruped_model::QuadrupedState::getJointPositions()
      .setSegment<QD::getNumDofLimb()>(QD::getLimbStartIndexInJ(limb),
                                       jointPositions);
}

void State::setAllJointPositions(const JointPositions &jointPositions) {
  quadruped_model::QuadrupedState::setJointPositions(
      quadruped_model::JointPositions(jointPositions.vector()));
}

const JointVelocitiesLeg
State::getJointVelocitiesForLimb(const LimbEnum &limb) const {
  return JointVelocitiesLeg(
      quadruped_model::QuadrupedState::getJointVelocities()
          .vector()
          .segment<QD::getNumDofLimb()>(QD::getLimbStartIndexInJ(limb)));
}

void State::setJointVelocitiesForLimb(
    const LimbEnum &limb, const JointVelocitiesLeg &jointVelocities) {
  quadruped_model::QuadrupedState::getJointVelocities()
      .setSegment<QD::getNumDofLimb()>(QD::getLimbStartIndexInJ(limb),
                                       jointVelocities);
}
...
void State::setAllJointVelocities(const JointVelocities &jointVelocities) {
  quadruped_model::QuadrupedState::setJointVelocities(
      quadruped_model::JointVelocities(jointVelocities.vector()));
}

const JointAccelerationsLeg
State::getJointAccelerationsForLimb(const LimbEnum &limb) const {
  return JointAccelerationsLeg(
      jointAccelerations_.vector().segment<QD::getNumDofLimb()>(
...
void State::setJointAccelerationsForLimb(
    const LimbEnum &limb, const JointAccelerationsLeg &jointAccelerations) {
  jointAccelerations_.setSegment<QD::getNumDofLimb()>(
      QD::getLimbStartIndexInJ(limb), jointAccelerations);
}

void State::setAllJointAccelerations(
    const JointAccelerations &jointAccelerations) {
  jointAccelerations_ = jointAccelerations;
}

const JointEffortsLeg
State::getJointEffortsForLimb(const LimbEnum &limb) const {
  return JointEffortsLeg(
      getAllJointEfforts().vector().segment<QD::getNumDofLimb()>(
          QD::getLimbStartIndexInJ(limb)));
}

const JointEfforts &State::getAllJointEfforts() const { return jointEfforts_; }

void State::setJointEffortsForLimb(const LimbEnum &limb,
                                   const JointEffortsLeg &jointEfforts) {
  jointEfforts_.getSegment<QD::getNumDofLimb()>(
...
void State::setControlSetup(const BranchEnum &branch,
                            const ControlSetup &controlSetup) {
  controlSetups_[branch] = controlSetup;
}

void State::setControlSetup(const LimbEnum &limb,
                            const ControlSetup &controlSetup) {
...
std::ostream &operator<<(std::ostream &out, const State &state) {
  out << static_cast<const quadruped_model::QuadrupedState &>(state)
      << std::endl;
  ...
  for (const auto &controlSetup : state.controlSetups_) {
    ...
    for (const auto &controlLevel : controlSetup.second) {
      if (controlLevel.second)
        out << controlLevel.first << ", ";
    }
    out << std::endl;
  }
  out << "Surface normals: " << state.surfaceNormals_ << std::endl;
  if (!state.stepId_.empty())
    out << "Step ID: " << state.stepId_ << std::endl;
  return out;
}
