...
StepCompleter::StepCompleter(const StepParameters &parameters,
                             const AdapterBase &adapter)
    : parameters_(parameters), adapter_(adapter) {}

StepCompleter::~StepCompleter() {}

bool StepCompleter::complete(const State &state, const StepQueue &queue,
                             Step &step) {
  for (auto &legMotion : step.legMotions_) {
    setParameters(*legMotion.second);
    legMotion.second->hasContactAtStart_ =
        adapter_.isLegGrounded(legMotion.first);
    ...
    case LegMotionBase::TrajectoryType::EndEffector:
      if (!complete(state, step,
                    dynamic_cast<EndEffectorMotionBase &>(*legMotion.second)))
        return false;
      break;
    case LegMotionBase::TrajectoryType::Joints:
      if (!complete(state, step,
                    dynamic_cast<JointMotionBase &>(*legMotion.second)))
        return false;
      break;
    default:
      throw std::runtime_error("StepCompleter::complete() could not complete "
                               "leg motion of this type.");
      break;
    }
  }
  ...
  if (step.baseMotion_) {
    ...
    if (!complete(state, step, queue, *(step.baseMotion_)))
      return false;
  }
...
bool StepCompleter::complete(const State &state, const Step &step,
                             EndEffectorMotionBase &endEffectorMotion) const {
  // Input.
  //  ControlSetup controlSetupIn =
  //  state.getControlSetup(endEffectorMotion.getLimb()); bool positionIn =
  //  controlSetupIn.at(ControlLevel::Position); bool velocityIn =
  //  controlSetupIn.at(ControlLevel::Velocity); bool accelerationIn =
  //  controlSetupIn.at(ControlLevel::Acceleration); bool effortIn =
  //  controlSetupIn.at(ControlLevel::Effort);
  ...
  if (positionOut) {
    ...
    Position startPositionInBaseFrame =
        adapter_.getPositionBaseToFootInBaseFrame(
            endEffectorMotion.getLimb(),
            state.getJointPositionsForLimb(endEffectorMotion.getLimb()));
    Position startPosition =
        adapter_.transformPosition("base", frameId, startPositionInBaseFrame);
  ...
  if (velocityOut) {
    ...
    const JointVelocitiesLeg &jointVelocities =
        state.getJointVelocitiesForLimb(endEffectorMotion.getLimb());
...
bool StepCompleter::complete(const State &state, const Step &step,
                             JointMotionBase &jointMotion) const {
  // Input.
  ...
  if (positionIn && !effortIn && positionOut && effortOut) {
    JointPositionsLeg startPosition =
        state.getJointPositionsForLimb(jointMotion.getLimb());
    jointMotion.updateStartPosition(startPosition);
    JointEffortsLeg startEffort =
        state.getJointEffortsForLimb(jointMotion.getLimb());
    startEffort.setZero();
    jointMotion.updateStartEfforts(startEffort);
  } else if (positionIn && effortIn && !positionOut && effortOut) {
    JointEffortsLeg startEffort =
        adapter_.getJointEffortsForLimb(jointMotion.getLimb());
    jointMotion.updateStartEfforts(startEffort);
  } // else if (positionIn && effortIn && positionOut && !effortOut) {
  ...
  // }
    ...
    if (velocityOut) {
      JointVelocitiesLeg startVelocity =
          state.getJointVelocitiesForLimb(jointMotion.getLimb());
      jointMotion.updateStartVelocity(startVelocity);
    }
    if (accelerationOut) {
      JointAccelerationsLeg startAcceleration =
          state.getJointAccelerationsForLimb(jointMotion.getLimb());
...
bool StepCompleter::complete(const State &state, const Step &step,
                             const StepQueue &queue,
                             BaseMotionBase &baseMotion) const {
  if (baseMotion.getControlSetup().at(ControlLevel::Position)) {
    ...
    Pose startPoseInWorld(state.getPositionWorldToBaseInWorldFrame(),
                          state.getOrientationBaseToWorld());
    Pose startPose = adapter_.transformPose(adapter_.getWorldFrameId(), frameId,
                                            startPoseInWorld);
...
void StepCompleter::setParameters(BaseAuto &baseAuto) const {
  ...
  baseAuto.nominalPlanarStanceInBaseFrame_ =
      parameters.nominalPlanarStanceInBaseFrame;
}
