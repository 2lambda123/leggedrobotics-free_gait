...
bool StepRosConverter::fromMessage(
    const std::vector<free_gait_msgs::Step> &message,
...
bool StepRosConverter::fromMessage(const free_gait_msgs::Step &message,
                                   Step &step) {
  // ID.
  ...
  for (const auto &footstepMessage : message.footstep) {
    ...
    if (!fromMessage(footstepMessage, footstep))
      return false;
  ...
  for (const auto &endEffectorTargetMessage : message.end_effector_target) {
    const auto limb =
        adapter_.getLimbEnumFromLimbString(endEffectorTargetMessage.name);
    EndEffectorTarget endEffectorTarget(limb);
    if (!fromMessage(endEffectorTargetMessage, endEffectorTarget))
      return false;
    step.addLegMotion(endEffectorTarget);
  }

  for (const auto &endEffectorTrajectoryMessage :
       message.end_effector_trajectory) {
    const auto limb =
        adapter_.getLimbEnumFromLimbString(endEffectorTrajectoryMessage.name);
    EndEffectorTrajectory endEffectorTrajectory(limb);
    if (!fromMessage(endEffectorTrajectoryMessage, endEffectorTrajectory))
      return false;
  ...
  for (const auto &legModeMessage : message.leg_mode) {
    ...
    if (!fromMessage(legModeMessage, legMode))
      return false;
  ...
  for (const auto &jointTrajectoryMessage : message.joint_trajectory) {
    const auto limb =
        adapter_.getLimbEnumFromLimbString(jointTrajectoryMessage.name);
    JointTrajectory jointTrajectory(limb);
    if (!fromMessage(jointTrajectoryMessage, jointTrajectory))
      return false;
  ...
  for (const auto &baseAutoMessage : message.base_auto) {
    BaseAuto baseAuto;
    if (!fromMessage(baseAutoMessage, baseAuto))
      return false;
  ...
  for (const auto &baseTargetMessage : message.base_target) {
    ...
    if (!fromMessage(baseTargetMessage, baseTarget))
      return false;
  ...
  for (const auto &baseTrajectoryMessage : message.base_trajectory) {
    ...
    if (!fromMessage(baseTrajectoryMessage, baseTrajectory))
      return false;
  ...
  for (const auto &customCommandMessage : message.custom_command) {
    ...
    if (!fromMessage(customCommandMessage, customCommand))
      return false;
...
bool StepRosConverter::fromMessage(const free_gait_msgs::Footstep &message,
  ...
  kindr_ros::convertFromRosGeometryMsg(message.surface_normal.vector,
                                       surfaceNormal);
...
bool StepRosConverter::fromMessage(
    const free_gait_msgs::EndEffectorTarget &message,
    EndEffectorTarget &endEffectorTarget) {
  // Limb.
  endEffectorTarget.limb_ = adapter_.getLimbEnumFromLimbString(message.name);

  // Target position.
  endEffectorTarget.controlSetup_[ControlLevel::Position] =
      !message.target_position.empty();
  if (endEffectorTarget.controlSetup_[ControlLevel::Position]) {
    endEffectorTarget.frameIds_[ControlLevel::Position] =
        message.target_position[0].header.frame_id;
    Position targetPosition;
    kindr_ros::convertFromRosGeometryMsg(message.target_position[0].point,
                                         targetPosition);
    endEffectorTarget.targetPosition_ = targetPosition;
  }

  // Target velocity.
  endEffectorTarget.controlSetup_[ControlLevel::Velocity] =
      !message.target_velocity.empty();
  if (endEffectorTarget.controlSetup_[ControlLevel::Velocity]) {
    endEffectorTarget.frameIds_[ControlLevel::Velocity] =
        message.target_velocity[0].header.frame_id;
    LinearVelocity targetVelocity;
    kindr_ros::convertFromRosGeometryMsg(message.target_velocity[0].vector,
                                         targetVelocity);
    endEffectorTarget.targetVelocity_ = targetVelocity;
  }

  // TODO.
  endEffectorTarget.controlSetup_[ControlLevel::Acceleration] =
      !message.target_acceleration.empty();
  endEffectorTarget.controlSetup_[ControlLevel::Effort] =
      !message.target_force.empty();
  ...
  // Friction coefficient.
  endEffectorTarget.frictionCoefficient_.reset(
      new double(message.friction_coefficient));

  // Ignore contact.
  endEffectorTarget.ignoreContact_ = message.ignore_contact;

  // Ignore for pose adaptation.
  endEffectorTarget.ignoreForPoseAdaptation_ =
      message.ignore_for_pose_adaptation;

  return true;
}

bool StepRosConverter::fromMessage(
    const free_gait_msgs::EndEffectorTrajectory &message,
    EndEffectorTrajectory &endEffectorTrajectory) {
  // Limb.
  endEffectorTrajectory.limb_ =
      adapter_.getLimbEnumFromLimbString(message.name);

  // Trajectory.
  endEffectorTrajectory.frameIds_[ControlLevel::Position] =
      message.trajectory.header.frame_id;

  for (const auto &point : message.trajectory.points) {
    if (!point.transforms.empty())
      endEffectorTrajectory.controlSetup_[ControlLevel::Position] = true;
    if (!point.velocities.empty())
      endEffectorTrajectory.controlSetup_[ControlLevel::Velocity] = true;
    if (!point.accelerations.empty())
      endEffectorTrajectory.controlSetup_[ControlLevel::Acceleration] = true;
  }

  for (const auto &controlSetup : endEffectorTrajectory.controlSetup_) {
    if (!controlSetup.second)
      continue;
    endEffectorTrajectory.values_[controlSetup.first] =
        std::vector<EndEffectorTrajectory::ValueType>();
  }

  // TODO Copy times correctly for pure velocity or acceleration trajectories.
  for (const auto &point : message.trajectory.points) {
    if (!point.transforms.empty()) {
      endEffectorTrajectory.times_.push_back(
          ros::Duration(point.time_from_start).toSec());
    } else {
      std::cerr << "StepRosConverter: Could not read from ROS message, only "
                   "position trajectories are supported for now."
                << std::endl;
      break;
    }
  }
  ...
  for (const auto &controlSetup : endEffectorTrajectory.controlSetup_) {
    if (!controlSetup.second)
      continue;
    for (const auto &point : message.trajectory.points) {
      if (controlSetup.first == ControlLevel::Position &&
          !point.transforms.empty()) {
        Position position;
        kindr_ros::convertFromRosGeometryMsg(point.transforms[0].translation,
                                             position);
        endEffectorTrajectory.values_[controlSetup.first].push_back(
            position.vector());
      } else if (controlSetup.first == ControlLevel::Velocity &&
                 !point.velocities.empty()) {
        //        baseTrajectory.derivatives_[controlSetup.first][j].push_back(point.velocities[j]);
      } else if (controlSetup.first == ControlLevel::Acceleration &&
                 !point.accelerations.empty()) {
        //        baseTrajectory.derivatives_[controlSetup.first][j].push_back(point.accelerations[j]);
      } /*else if (controlSetup.first == ControlLevel::Effort &&
!point.effort.empty()) {
  ...
  // Friction coefficient.
  endEffectorTrajectory.frictionCoefficient_.reset(
      new double(message.friction_coefficient));
  ...
  // Ignore for pose adaptation.
  endEffectorTrajectory.ignoreForPoseAdaptation_ =
      message.ignore_for_pose_adaptation;

  return true;
}

bool StepRosConverter::fromMessage(const free_gait_msgs::LegMode &message,
                                   LegMode &legMode) {
  // Limb.
...
bool StepRosConverter::fromMessage(
    const free_gait_msgs::JointTrajectory &message,
  ...
  for (const auto &jointName : message.trajectory.joint_names) {
    jointTrajectory.jointNodeEnums_.push_back(
        adapter_.getJointNodeEnumFromJointNodeString(jointName));
  }

  for (const auto &point : message.trajectory.points) {
    if (!point.positions.empty())
      jointTrajectory.controlSetup_[ControlLevel::Position] = true;
    if (!point.velocities.empty())
      jointTrajectory.controlSetup_[ControlLevel::Velocity] = true;
    if (!point.accelerations.empty())
      jointTrajectory.controlSetup_[ControlLevel::Acceleration] = true;
    if (!point.effort.empty())
      jointTrajectory.controlSetup_[ControlLevel::Effort] = true;
  }

  for (const auto &controlSetup : jointTrajectory.controlSetup_) {
    if (!controlSetup.second)
      continue;
    jointTrajectory.times_[controlSetup.first] =
        std::vector<JointTrajectory::Time>();
    jointTrajectory.values_[controlSetup.first] =
        std::vector<std::vector<JointTrajectory::ValueType>>();
    jointTrajectory.trajectories_[controlSetup.first] =
        std::vector<curves::PolynomialSplineQuinticScalarCurve>();
  }

  for (const auto &point : message.trajectory.points) {
    if (!point.positions.empty())
      jointTrajectory.times_[ControlLevel::Position].push_back(
          ros::Duration(point.time_from_start).toSec());
    if (!point.velocities.empty())
      jointTrajectory.times_[ControlLevel::Velocity].push_back(
          ros::Duration(point.time_from_start).toSec());
    if (!point.accelerations.empty())
      jointTrajectory.times_[ControlLevel::Acceleration].push_back(
          ros::Duration(point.time_from_start).toSec());
    if (!point.effort.empty())
      jointTrajectory.times_[ControlLevel::Effort].push_back(
          ros::Duration(point.time_from_start).toSec());
  }
  ...
  for (const auto &controlSetup : jointTrajectory.controlSetup_) {
    for (size_t j = 0; j < nJoints; ++j) {
      if (!controlSetup.second)
        continue;
      jointTrajectory.values_[controlSetup.first].push_back(
          std::vector<JointTrajectory::ValueType>());
      for (const auto &point : message.trajectory.points) {
        if (controlSetup.first == ControlLevel::Position &&
            !point.positions.empty()) {
          jointTrajectory.values_[controlSetup.first][j].push_back(
              point.positions[j]);
        } else if (controlSetup.first == ControlLevel::Velocity &&
                   !point.velocities.empty()) {
          jointTrajectory.values_[controlSetup.first][j].push_back(
  ...
  // Friction coefficient.
  jointTrajectory.frictionCoefficient_.reset(
      new double(message.friction_coefficient));
...
bool StepRosConverter::fromMessage(
    const free_gait_msgs::BaseTrajectory &message,
    BaseTrajectory &baseTrajectory) {
  // Trajectory.
  baseTrajectory.frameIds_[ControlLevel::Position] =
      message.trajectory.header.frame_id;
  ...
  for (const auto &point : message.trajectory.points) {
    if (!point.transforms.empty())
      baseTrajectory.controlSetup_[ControlLevel::Position] = true;
    if (!point.velocities.empty())
      baseTrajectory.controlSetup_[ControlLevel::Velocity] = true;
    if (!point.accelerations.empty())
      baseTrajectory.controlSetup_[ControlLevel::Acceleration] = true;
  }

  if (baseTrajectory.controlSetup_[ControlLevel::Position]) {
    baseTrajectory.values_[ControlLevel::Position] =
        std::vector<BaseTrajectory::ValueType>();
  }
  for (const auto &controlSetup : baseTrajectory.controlSetup_) {
    if (controlSetup.first == ControlLevel::Position || !controlSetup.second)
      continue;
    baseTrajectory.times_[controlSetup.first] =
        std::vector<BaseTrajectory::Time>();
    baseTrajectory.derivatives_[controlSetup.first] =
        std::vector<BaseTrajectory::DerivativeType>();
  }

  for (const auto &point : message.trajectory.points) {
    if (!point.transforms.empty())
      baseTrajectory.times_[ControlLevel::Position].push_back(
          ros::Duration(point.time_from_start).toSec());
    if (!point.velocities.empty())
      baseTrajectory.times_[ControlLevel::Velocity].push_back(
          ros::Duration(point.time_from_start).toSec());
    if (!point.accelerations.empty())
      baseTrajectory.times_[ControlLevel::Acceleration].push_back(
          ros::Duration(point.time_from_start).toSec());
  }
...
bool StepRosConverter::fromMessage(const free_gait_msgs::CustomCommand &message,
                                   CustomCommand &customCommand) {
...
bool StepRosConverter::toMessage(
    const StepQueue &stepQueue,
    free_gait_msgs::ExecuteStepsGoal::_steps_type &message) {
  for (const Step &step : stepQueue.getQueue()) {
    free_gait_msgs::Step stepMessage;
    if (!toMessage(step, stepMessage))
      return false;
...
bool StepRosConverter::toMessage(const Step &step,
                                 free_gait_msgs::Step &message) {
  ...
  for (const auto &legMotion : step.getLegMotions()) {
    ...
    if (legMotion.second->getType() == LegMotionBase::Type::Footstep) {
      const Footstep &footstep =
          dynamic_cast<const Footstep &>(*(legMotion.second));
      free_gait_msgs::Footstep message;
      if (!toMessage(footstep, message))
        return false;
      stepMessage.footstep.push_back(message);
    }

    // EndEffectorTajectory
    if (legMotion.second->getType() ==
        LegMotionBase::Type::EndEffectorTrajectory) {
      const EndEffectorTrajectory &endEffectorTrajectory =
          dynamic_cast<const EndEffectorTrajectory &>(*(legMotion.second));
      free_gait_msgs::EndEffectorTrajectory message;
      if (!toMessage(endEffectorTrajectory, message))
        return false;
    ...
    if (legMotion.second->getType() == LegMotionBase::Type::JointTrajectory) {
      const JointTrajectory &jointTrajectory =
          dynamic_cast<const JointTrajectory &>(*(legMotion.second));
      free_gait_msgs::JointTrajectory message;
      if (!toMessage(jointTrajectory, message))
        return false;
  ...
  if (step.hasBaseMotion()) {
    ...
    if (baseMotion.getType() == BaseMotionBase::Type::Trajectory) {
      const BaseTrajectory &baseTrajectory =
          dynamic_cast<const BaseTrajectory &>(baseMotion);
      free_gait_msgs::BaseTrajectory message;
      if (!toMessage(baseTrajectory, message))
        return false;
  ...
  for (const auto &customCommand : step.getCustomCommands()) {
    ...
    if (!toMessage(customCommand, message))
      return false;
...
bool StepRosConverter::toMessage(const Footstep &footstep,
                                 free_gait_msgs::Footstep &message) {
  // Limb.
  ...
  if (footstep.surfaceNormal_) {
    kindr_ros::convertToRosGeometryMsg(*(footstep.surfaceNormal_),
                                       message.surface_normal.vector);
  }
...
bool StepRosConverter::toMessage(
  ...
  // Limb.
  message.name =
      adapter_.getLimbStringFromLimbEnum(endEffectorTrajectory.limb_);

  // Trajectory.
  message.trajectory.header.frame_id =
      endEffectorTrajectory.frameIds_.at(ControlLevel::Position);
  ...
  for (auto &point : message.trajectory.points) {
    ...
    if (endEffectorTrajectory.controlSetup_.at(ControlLevel::Position)) {
      ...
      Position position(
          endEffectorTrajectory.values_.at(ControlLevel::Position)[i]);
  ...
  if (endEffectorTrajectory.frictionCoefficient_) {
    message.friction_coefficient =
        *(endEffectorTrajectory.frictionCoefficient_);
  }
...
bool StepRosConverter::toMessage(const JointTrajectory &jointTrajectory,
                                 free_gait_msgs::JointTrajectory &message) {
  // Limb.
  ...
  if (jointTrajectory.surfaceNormal_) {
    kindr_ros::convertToRosGeometryMsg(*(jointTrajectory.surfaceNormal_),
                                       message.surface_normal.vector);
  }
  ...
  for (const auto &jointNode : jointTrajectory.jointNodeEnums_) {
    message.trajectory.joint_names.push_back(
        adapter_.getJointNodeStringFromJointNodeEnum(jointNode));
  }
    ...
    if (controlLevel.second) {
      ...
      for (const auto &time : jointTrajectory.times_.at(controlLevel.first)) {
        std::vector<trajectory_msgs::JointTrajectoryPoint>::iterator it =
            std::find_if(
                message.trajectory.points.begin(),
                message.trajectory.points.end(),
                [&](const trajectory_msgs::JointTrajectoryPoint &point)
                    -> bool { return point.time_from_start.toSec() >= time; });
        if (it == message.trajectory.points.end() ||
            it->time_from_start.toSec() != time) {
          it = message.trajectory.points.insert(
              it, trajectory_msgs::JointTrajectoryPoint());
...
bool StepRosConverter::toMessage(const BaseAuto &baseAuto,
                                 free_gait_msgs::BaseAuto &message) {
  if (baseAuto.height_)
...
bool StepRosConverter::toMessage(const BaseTrajectory &baseTrajectory,
                                 free_gait_msgs::BaseTrajectory &message) {
  message.trajectory.header.frame_id =
      baseTrajectory.frameIds_.at(ControlLevel::Position);
  message.trajectory.points.resize(
      baseTrajectory.values_.at(ControlLevel::Position).size());
  size_t i = 0;
  for (auto &point : message.trajectory.points) {
    if (baseTrajectory.controlSetup_.at(ControlLevel::Position)) {
      point.time_from_start =
          ros::Duration(baseTrajectory.times_.at(ControlLevel::Position)[i]);
      geometry_msgs::Transform transform;
      BaseTrajectory::ValueType pose(
          baseTrajectory.values_.at(ControlLevel::Position)[i]);
...
bool StepRosConverter::toMessage(const CustomCommand &customCommand,
                                 free_gait_msgs::CustomCommand &message) {
